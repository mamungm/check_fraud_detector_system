...
MODEL_DIR = "models"
MODEL_PATH = f"{MODEL_DIR}/in_clearing_fraud_model.joblib"
METRICS_PATH = f"{MODEL_DIR}/in_clearing_fraud_metrics.json"
...
def main():
    ...
    required = ["event_id", "institution_id", TARGET] + IN_CLEARING_FEATURES + CATEGORICAL_FEATURES
    ...
    for c in IN_CLEARING_FEATURES:
        df[c] = pd.to_numeric(df[c], errors="coerce").fillna(0)

    df["channel"] = df["channel"].fillna("unknown").astype(str)

    X = df[IN_CLEARING_FEATURES + CATEGORICAL_FEATURES]
    ...
    X_train = train_df[IN_CLEARING_FEATURES + CATEGORICAL_FEATURES]
    X_test = test_df[IN_CLEARING_FEATURES + CATEGORICAL_FEATURES]

    preprocessor = ColumnTransformer(
        transformers=[
            ("cat", OneHotEncoder(handle_unknown="ignore"), CATEGORICAL_FEATURES),
            ("num", "passthrough", IN_CLEARING_FEATURES),
        ]
    )
